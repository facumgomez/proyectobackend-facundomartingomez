...
export const getCartById = async (req, res) => {
  try {
    ...
    if(!cart)
      return res.status(404).json({ status: 'error', message: `Carrito no encontrado con ID ${cid}` });
    res.status(200).json(cart);
  } catch (error) {
    if (error.name === 'ErrorCast') 
      return res.status(404).json({ status: 'error', message: 'No hay ningún carrito con ese ID' });
...
export const createCart = async (req, res) => {
  try {
    ...
    return res.status(200).json({ status: 'success', message: 'Carrito creado', newCart });
  } catch (error) {
    res.status(404).json({ status: 'error', message: 'El documento no tiene un formato válido.' });
  };
};
...
export const addProductInCart = async (req, res) => {
  try {
    ...
    if (!cart)
      return res.status(404).json({ status: 'error', message: `Carrito no encontrado con ID ${cid}` });
    res.status(200).json({ status: 'success', message: 'El producto ha sido actualizado en el carrito.', cart });
  } catch (error) {
    if (error.name === 'ErrorCast') 
      return res.status(404).json({ status: 'error', message: 'No hay ningún carrito con ese ID' });
    res.status(404).json({ status: 'error', message: 'El documento no tiene un formato válido.' });
...
export const updateProductQuantity = async (req, res) => {
  try {
    ...
    if (!cart)
      return res.status(404).json({ status: 'error', message: 'La cantidad se pudo actualizar' });
